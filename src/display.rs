...
use anyhow::Result;
use linecheck::{checker::{check_file, CheckOptions}, result::{FileResult, Status}, config::ConfigResolver};

fn run<F: FnMut(&PathBuf, FileResult)>(files: &[PathBuf], resolver: &mut ConfigResolver, opts: &CheckOptions, mut each: F) -> Result<()> {
    for f in files { match check_file(f, resolver.resolve(f).as_ref(), opts) { Ok(r) => each(f, r), Err(e) => eprintln!("Error: {e}") } }
    Ok(())
}
...
pub fn print_violations(files: &[PathBuf], resolver: &mut ConfigResolver, opts: &CheckOptions, has_error: &mut bool) -> Result<()> {
    run(files, resolver, opts, |file, r| {
        ...
        println!("{}: {} lines{}{}", file.display(), r.lines, lim.map_or(String::new(), |t| format!(" ({kind} threshold: {t})")), r.message.as_deref().map_or(String::new(), |m| format!(" — {m}")));
...
pub fn print_status(files: &[PathBuf], resolver: &mut ConfigResolver, opts: &CheckOptions, has_error: &mut bool) -> Result<()> {
    ...
    run(files, resolver, opts, |file, r| {
        let Some(limit) = (if r.status == Status::Warn { r.warn_limit.or(r.error_limit) } else { r.error_limit.or(r.warn_limit) }) else { return };
        if r.status == Status::Error { *has_error = true; }
    ...
    let (lw, tw) = (rows.iter().map(|r| digits(r.lines)).max().unwrap_or(0), rows.iter().map(|r| digits(r.limit)).max().unwrap_or(0));
    for row in &rows {
        let pct = if row.limit > 0 { row.lines * 100 / row.limit } else { 0 };
        let tag = match row.status { Status::Error => "[ERROR]".to_owned(), Status::Warn => "[WARN]".to_owned(), _ => format!("{pct}%") };
...
pub fn print_json(files: &[PathBuf], resolver: &mut ConfigResolver, opts: &CheckOptions, status_mode: bool, has_error: &mut bool) -> Result<()> {
    ...
    run(files, resolver, opts, |file, r| {
        ...
        items.push(format!(r#"  {{"file":{f},"lines":{l},"limit":{lim},"percent":{pct},"status":"{st}"{msg}}}"#, f = serde_json::to_string(&file.display().to_string()).unwrap_or_else(|_| "null".into()), l = r.lines));
    })?;
