...
fn main() -> Result<()> {
    let args = Args::parse();
    let is_default_config_name = args.config.strip_prefix(".").unwrap_or(&args.config) == std::path::Path::new("linecheck.yml");
    ...
    let preset = args.strict.then_some(Preset::Strict)
